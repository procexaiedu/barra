"""Setup de observabilidade: Langfuse self-hosted (ADR 0019) + Sentry.

O tracing de produção é o Langfuse self-hosted, SEM masking — o Langfuse roda no mesmo perímetro
de confiança do banco que já guarda a PII, então a proteção migrou do masking-no-egress para o
controle de acesso (ADR 0019). O caminho LangSmith (`setup_tracing` + anonymizer) foi retirado no
cutover da Fase 3 e não existe mais.
"""
...
# Atendimento (uma negociação cliente-modelo), não a Conversa cliente — o Langfuse agrupa o
...
def setup_langfuse(settings: Settings, *, servico: str = "barra") -> Any | None:
    ...
    Único caminho de tracing do `main.py`/worker (substituiu o LangSmith, retirado). SEM masking: com o
...
def metadata_trace_turno(modelo_id: str, atendimento_id: str, cliente_id: str) -> dict[str, Any]:
    ...
    viajam como metadata/tags do run — filtra/agrupa traces por modelo, por
...
def init_sentry(settings: Settings) -> bool:
    ...
    sentry_sdk.init(
        ...
        # PII hard-gate: sem isso o SDK serializa
