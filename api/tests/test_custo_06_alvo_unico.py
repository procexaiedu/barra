...
Antes o numero 0.12 estava duplicado em comentarios/help de core/metrics.py, agente/nos/llm.py
e _custo.py. Esta suite trava o contrato: (1) o campo existe com o default documentado (0.12) e
...
def test_custo_alvo_brl_default_documentado() -> None:
    # 0.12 e a meta documentada no help do Histogram de custo do turno (03 §4.2).
    assert Settings().custo_alvo_brl == 0.12
...
def test_help_do_histogram_aponta_pro_settings_sem_numero() -> None:
    ...
    assert "0.12" not in doc


def test_nenhum_modulo_de_custo_hardcoda_o_numero() -> None:
    # Guard contra regressao: o literal 0.12 nao pode voltar aos modulos que so observam/calculam
    ...
    for rel in ("core/metrics.py", "agente/nos/llm.py", "agente/_custo.py"):
        ...
        assert "0.12" not in texto, f"{rel} reintroduziu o alvo de custo hardcoded"
