"""Validacao de schema das fixtures de leitura (DB-free, sem chave da Anthropic).

Garante que cada .jsonl em evals/canonicos/leitura/ e JSON valido por linha, carrega as chaves
criticas do schema (evals/README.md; 08 §2.4), que tool_calls_obrigatorias/proibidas sao listas
disjuntas, e que os `id` sao unicos entre os arquivos. NAO roda o agente nem toca o banco -- e o
gate barato que pega fixture quebrada antes do runner do M6 (que ainda nao existe).
"""
...
def test_ha_fixtures() -> None:
    assert _LINHAS, f"nenhuma fixture .jsonl encontrada em {_LEITURA}"
...
def test_schema_da_fixture(label: str, raw: str) -> None:
    ...
    for chave in ("id", "categoria", "subcategoria", "expectativas", "rubricas"):
