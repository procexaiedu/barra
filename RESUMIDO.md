...
- `<plano_externo_atipico>` **reescrito (feedback Fernando 09/06)**: o passo "peça orçamento"
  saiu — pedir orçamento inverte a autoridade de preço e quebra o posicionamento de luxo (o
  cliente contrata o programa DELA; contradizia a própria `<cotacao>`, que manda cotar direto).
  Novo fluxo: vende vontade → dimensiona o TEMPO ("até que horas vc quer comigo?"), nunca o
  dinheiro → cota o pacote correspondente da tabela dela (pernoite/multi-hora) → cliente que
  oferece valor por conta própria cai na régua de `<desconto>` (uma contraproposta, depois
  escala). Exemplos atualizados.
...
- Contexto (feedback Fernando 09/06): os programas/valores JÁ são puxados da interface a cada
  turno — Fernando cadastra no painel (`POST/PATCH /modelos/{id}/programas` → `modelo_programas`)
  e o `prepare_context` consulta essas tabelas e renderiza a tabela de valores no bloco
  por-modelo do prompt. O ramo "sem programas" é só fallback defensivo para cadastro incompleto.
- Esse fallback foi reescrito para deixar isso explícito: "estado anormal de cadastro
  incompleto, não uma característica sua", NUNCA inventar/chutar valor, seguir qualificando e
  `escalar(motivo="politica_nova_necessaria")` se perguntarem preço. (Antes dizia "A modelo
  ainda não tem programas" — 3ª pessoa e tom de estado normal.) Teste `test_bp3_render.py::
  test_programas_vazio_orienta_escalar` ajustado para os novos fragmentos.
...
1. **Custo de cache no deploy**: mudar persona/regras/FAQ/docstrings invalida o cache do prefixo
...
2. **Evals ao vivo**: as mudanças são de redação/consistência, sem intenção de mudar conduta, mas
