...
_Avoid_: tratar interno como localização do cliente; a IA revelar a **unidade** (apto/quarto) do interno (só a modelo passa, pós-Foto de portaria); passar rua+número do prédio antes de haver intenção real (no 1º contato/sondagem, só a região); exigir Pix no interno/remoto ou Foto de portaria no externo/remoto; exigir Pix quando o cliente busca a modelo; plotar interno ou remoto no Mapa; misturar remoto e presencial no mesmo atendimento.
