  ...
  return (
    <div className="rounded-md border border-border bg-card px-3 py-2 text-[12px] shadow-lg shadow-black/40">
      ...
      <dl className="grid grid-cols-[auto_auto] gap-x-3 gap-y-0.5">
        ...
        <dd className="text-right font-mono tabular-nums" style={{ color: "var(--success-500)" }}>
          {formatBRL(p.valor)}
        </dd>
        <dt className="text-text-muted">Lançamentos</dt>
        <dd className="text-right font-mono tabular-nums text-text-secondary">{p.qtd}</dd>
        {p.modelosDistintas > 1 && (
          <>
            <dt className="text-text-muted">Modelos</dt>
            <dd className="text-right font-mono tabular-nums text-text-secondary">{p.modelosDistintas}</dd>
          </>
        )}
      </dl>
    </div>
