...
- Gasto de crédito de LLM real: tudo que chama LLM roda **DeepSeek V4 Flash direto** (`api.deepseek.com`) — agente ao vivo, `make test-llm` e os evals; não há mais caminho Anthropic no repo.
...
- `make test` — pytest
- `make test-llm` — testes que batem na API real (consome crédito DeepSeek; ver gate)
