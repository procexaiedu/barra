...
- `gera_indice_evals.py` — gera `docs/agente/evals-fixtures-indice.html`, indice navegavel das fixtures de `api/evals/{canonicos,adversariais}` com selo BARRA/AVISA (gate espelhado do `runner.py`), em linguagem simples. Regenere quando as fixtures mudarem.
- `evals_resumos.json` — descricoes em linguagem simples por fixture (id -> texto), curado a mao e usado pelo `gera_indice_evals.py`; se faltar um id, cai na descricao tecnica do `.jsonl`.
