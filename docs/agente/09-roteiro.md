...
- **Objetivo:** stack de eval offline em `evals/runners/`, integrando **`agentevals`** (dev dep, lib oficial LangChain) com driver custom Barra:
  ...
  - `judge.md` — prompt do LLM-judge passado para `agentevals.trajectory.llm.create_trajectory_llm_as_judge(model="anthropic:claude-sonnet-4-6", prompt=...)`, cobrindo as 5 rubricas-piso (`persona`/`tom_pt_br`/`instruction_following` LLM + `tool_use_correto`/`isolamento_par` determinísticas) e mais duas **opt-in por categoria** (ver catálogo `08 §2.2`): `deve_negar_identidade` (rubrica AUP vinculante, camada 2 do gate, `08 §2.2`, em `adversariais/disclosure/` + canônicas AUP-críticas) e `coerencia_multiturno` (consistência narrativa entre turnos, em fixtures ≥3 turnos do cliente — gaslighting/cross_modelo/recorrente/`scripted_5` longas). AUP exige score 1.0 (binário); qualquer judge-flag → revisão humana antes do cutover (`08 §4.1`).
...
- **Aceite:** todas as fixtures validam contra o schema; cobertura mínima por categoria atingida; **cada uma das 4 transcrições de `conversas-reais/` está representada em ≥1 fixture canônica** (rastreável via campo `descricao` ou metadata `origem_conversa_real`); fixtures de `adversariais/disclosure/` + canônicas AUP-críticas declaram `deve_negar_identidade` em `rubricas`; fixtures com ≥3 turnos do cliente (gaslighting, cross_modelo, recorrente, `scripted_5` longas) declaram `coerencia_multiturno` em `rubricas` (ver catálogo `08 §2.2`).
