...
| [12-observabilidade-langfuse.md](12-observabilidade-langfuse.md) | **O trace do turno no Langfuse**: o que o root span traz de relance (fala, desfecho, **raciocínio do thinking**), vocabulário de tags (escopo/regime/desfecho), nomes das generations, scores, receitas de consulta via MCP e os limites do que o trace responde | Investigando um caso do agente, comparando braços de A/B, ou mexendo em `core/tracing.py` |

## Stack do agente (resumo)

- **Provider LLM (texto):** **DeepSeek V4 Flash DIRETO** (`api.deepseek.com`, OpenAI-compatível) via `langchain-openai.ChatOpenAI` (`core/llm.py:criar_chat_deepseek`). É o único provider dos **3 caminhos de texto** do agente ao vivo: chat #1, extração forçada #2 e judge de AUP #3. Desde **11/08/2026** o **chat #1 roda thinking** (`reasoning_effort` de `settings.deepseek_thinking_chat`, default `low`; `DEEPSEEK_THINKING_CHAT=disabled` volta ao regime non-thinking + temperatura) — extração #2 e judge #3 seguem travados em `disabled` (thinking corromperia o structured output). Cache automático de prefixo no endpoint oficial (sem `cache_control`).
...
- **Tracing:** **Langfuse self-hosted** (ADR 0019, `core/tracing.py:setup_langfuse`) — o LangSmith foi retirado. Anatomia do trace do turno, vocabulário de tags e receitas de consulta: [12-observabilidade-langfuse.md](12-observabilidade-langfuse.md).
