...
- **Gate uniforme em `tipo_atendimento_aceito[]`.** A modelo habilita vídeo chamada tendo `remoto` em `tipo_atendimento_aceito[]` **e** o programa Vídeo chamada em `modelo_programas` (preço/duração) — a IA checa `aceito[]` para nunca negociar um tipo que a modelo não faz, exatamente como já faz para interno/externo. O vínculo "programa Vídeo chamada ⟹ `remoto`" fica por **convenção** (prompt + extração), sem constraint de banco no P0.
  ...
  > **Nota (2026-08-14) — a convenção não se sustentou; o programa passou a habilitar sozinho.** Os dois campos são independentes e nunca tiveram validação cruzada, e as camadas divergiram: o **prompt** decide se vende pelo CARDÁPIO (`prepare_context._carregar_bp3` → `sem_video_chamada`, derivado de `core.catalogo.e_video_chamada` sobre `modelo_programas`) e o **domínio** decidia se aceita só pelo CHECKBOX. Com `checkbox off + programa presente`, a IA vendia e cotava a chamada enquanto toda extração `remoto` era descartada (antes do ciclo 7: **escalada + IA pausada**) — a venda acontecia na conversa e não existia no sistema, e o trilho decidido aqui (bloqueio prévio, cron `confirmar_em_execucao`, card, Pix do ADR-0029) nunca armava. Desde então `_tipo_aceito` (`dominio/atendimentos/service.py`) aceita `remoto` **por união**: checkbox **ou** linha de vídeo chamada no cardápio. O checkbox continua valendo sozinho (ninguém perde comportamento) e `agente_cadastro_remoto_incoerente_total{modo}` mede os dois lados divergindo. **Pendente do dono:** (a) manter a união, exigir os dois (AND) ou **matar o checkbox** e derivar de vez (é a opção que elimina a classe inteira, e custa migration de backfill + ajuste do painel — `DialogCriarModelo.tsx`); (b) o modo inverso, `checkbox on + programa ausente`, segue aceito de propósito, apesar de o belief passar a dizer "remoto" numa conversa em que a IA acabou de negar a chamada.
