...
> **Emenda 2026-07-25:** o **degrau do número deixou de ser prosa** e virou o segundo degrau do mesmo gate estrutural (`prepare_context._libera_numero_do_endereco`): em `Qualificado` o bloco `<local_de_encontro>` renderiza o `endereco_formatado` **sem o número** (remoção determinística, preservando logradouro, bairro e cidade); o número entra de `Aguardando_confirmacao` em diante. Reverte a escolha de deixá-lo em prosa — o motivo dado lá ("o subcaso 'confirmou que vai' acontece no mesmo estado") não se sustentou: **é o horário combinado que promove a `Aguardando_confirmacao`**, então "confirmou que vai" e "avisou que saiu" (só carimbado nesse estado) já caem do lado liberado. O que o gate aproxima é o subcaso "pediu pra se organizar" **sem** horário cravado: ali a IA não recebe o número e o bloco a instrui a cravar o encontro para destravá-lo — o que empurra a venda em vez de esconder um dado que ela tem. Motivo da reversão: o 1º dia de prod mostrou que a prosa de degraus não segura o DeepSeek, e o que ela não recebe ela não vaza.
