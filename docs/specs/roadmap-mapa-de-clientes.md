...
| MAPA-14 | Comparar dois períodos (lift de campanha) | extra | 🟡 | MAPA-6 | ✅ shipado em sequência (pós-MAPA-11) |
...
- **MAPA-14 shipado em 2026-05-26** em branch `feat/clientes-mapa-14-comparar-periodos`: toggle Comparar + 2 pares de `<input type="date">` no `MapaToolbar` (sem dep nova). Endpoint estendido com `comparar=true` + `a_inicio/a_fim/b_inicio/b_fim` (ISO `YYYY-MM-DD`); query é UNION ALL de duas LATERAL `geo`/`ag` por recorte, 1 ponto por par (cliente, recorte) com `recorte: "A"|"B"`. Validação `fim < inicio` → 422. Modo Comparar força `camadaEfetiva = hexbin` (sobrescreve sem mutar a escolha do usuário), troca para paleta **divergente** BrBG invertida (helper novo `lib/cores/divergente.ts`) e ativa `getColorValue` com `colorDomain` simétrico em torno de zero (`somaDeltasPorCelula` estima a cota superior). InfoWindow do favo mostra `A`, `B`, `Δ` quando comparar=on. SeletorRecencia e Camada (Bolhas/Calor) ficam disabled durante Comparar. 5 testes pytest novos (cliente só em A; cliente em A+B com agregados do recorte; recorte vazio; range invertido 422; ignora `periodo`/`recencia`). Spec Playwright dispensada por solicitação do usuário; verificação por typecheck/lint/build (limpos) + pytest (21 passed).
