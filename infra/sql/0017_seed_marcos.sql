...
-- === bloqueios ===
-- ATD_MARC_1 não tem bloqueio: ainda em Triagem, horário não confirmado.

-- === atendimentos (sem bloqueio_id) ===

-- ATD_MARC_1: Triagem hoje, interno, urgência imediato, sem horário/duração confirmados
INSERT INTO barravips.atendimentos (
  ...
  '91000000-0000-0000-0000-000000000007', 7,
  'c1000000-0000-0000-0000-000000000005',
  'a1000000-0000-0000-0000-000000000001',
  'f1000000-0000-0000-0000-000000000005',
  NULL,
  'Triagem', 'interno', 'imediato',
  NOW()::date, NULL, NULL,
  NULL, NULL, NULL, NULL,
  NULL, NULL, NULL, NULL,
  ...
  NULL, NULL,
  false, NULL, 'IA',
  'IA coletar horário, local e programa desejados.',
  NULL,
  'Marcos Lima, novo. Demonstrou interesse; IA em triagem.',
  '{"informa_horario":false,"informa_local":false,"aceita_valor":false,"envia_pix":false,"responde_objetivamente":true}',
  'extracao_ia',
  NOW() - INTERVAL '1 hour',
...
-- === mensagens ===
-- CNV_MARC_ALE — MSG-042 a MSG-045 (ATD_MARC_1, Triagem)
...
(
  -- MSG-042: Marcos abre a conversa
  '0a500000-0000-0000-0000-000000000042',
  'f1000000-0000-0000-0000-000000000005', '91000000-0000-0000-0000-000000000007',
  'cliente', 'texto',
  'Oi, você está disponível hoje?',
  NULL, '3EB0MARC00000001',
  NOW() - INTERVAL '1 hour'
),
(
  -- MSG-043: IA Alessia responde estimulando qualificação
  '0a500000-0000-0000-0000-000000000043',
  'f1000000-0000-0000-0000-000000000005', '91000000-0000-0000-0000-000000000007',
  'ia', 'texto',
  'Oi! Estou sim 😊 O que você tem em mente? Me conta mais!',
  NULL, '3EB0MARC00000002',
  NOW() - INTERVAL '58 minutes'
),
(
  -- MSG-044: Marcos pergunta sobre local (sinal de atendimento interno)
  '0a500000-0000-0000-0000-000000000044',
  'f1000000-0000-0000-0000-000000000005', '91000000-0000-0000-0000-000000000007',
  'cliente', 'texto',
  'Queria ir aí visitar... é no apartamento de vocês mesmo?',
  NULL, '3EB0MARC00000003',
  NOW() - INTERVAL '55 minutes'
),
(
  -- MSG-045: IA confirma interno e pede horário (continua triagem)
  '0a500000-0000-0000-0000-000000000045',
  'f1000000-0000-0000-0000-000000000005', '91000000-0000-0000-0000-000000000007',
  'ia', 'texto',
  'É sim, aqui mesmo, bem tranquilo 😊 Em qual horário você pensou?',
  NULL, '3EB0MARC00000004',
  NOW() - INTERVAL '53 minutes'
)
ON CONFLICT (id) DO NOTHING;

-- === comprovantes_pix ===
-- Nenhum: Marcos ainda em Triagem (pix_status='nao_solicitado').

-- === escaladas ===
-- Nenhuma: IA conduz triagem normalmente, sem handoff.

-- === eventos ===
-- ATD_MARC_1: Novo → Triagem (única transição até agora)
INSERT INTO barravips.eventos (id, atendimento_id, tipo, origem, autor, payload, created_at) VALUES
(
  -- EN1: IA detecta abertura do atendimento e classifica como Triagem
  '0e500000-0000-0000-0000-000000000001',
...
-- === envios_evolution ===
-- MSG-043: IA Alessia responde Marcos (único envio backend nesta conversa até agora)
...
(
  'ef050000-0000-0000-0000-000000000001',
  '3EB0IA000000007',
  'evo_alessia',
  '5521999990005@s.whatsapp.net',
  'conversa_cliente', 'outbound_backend', 'ia',
  '91000000-0000-0000-0000-000000000007', 'f1000000-0000-0000-0000-000000000005',
  '{"tipo_msg":"texto","len":47}',
  NOW() - INTERVAL '58 minutes'
),
(
  -- MSG-045: IA pede horário para concluir a triagem
  'ef050000-0000-0000-0000-000000000002',
  '3EB0MARCIA000001',
  'evo_alessia',
  '5521999990005@s.whatsapp.net',
  'conversa_cliente', 'outbound_backend', 'ia',
  '91000000-0000-0000-0000-000000000007', 'f1000000-0000-0000-0000-000000000005',
  '{"tipo_msg":"texto","len":63}',
  NOW() - INTERVAL '53 minutes'
)
ON CONFLICT (id) DO NOTHING;

-- === atendimento_servicos ===
-- Nenhum: ATD_MARC_1 está em Triagem (não Fechado).
