...
Mapeia os termos do `CONTEXT.md` para a representação visual canônica. Esta é a tradução obrigatória — alterar a cor de "Pix em revisão" ou inventar variante para "Handoff" sem atualizar este arquivo é regressão.
...
| Modelo em atendimento | `paused` | `border-l-info-500` | `text-state-paused` | `ClockAlert` |
| Registro de resultado fechado | `closed` | — | `text-state-closed` (success) | — |
| Registro de resultado perdido | `lost` | — | `text-state-lost` (danger) | — |

Fonte canônica: `CardDestaque.tsx:25-29` (`BADGE_MAP`). Quando adicionar novo `IaPausadaMotivo`, atualizar aqui e o mapa lá juntos.

## Urgência temporal

Pattern em `CardDestaque.tsx:31-36` (`urgenciaClasse`):
...
Pattern em `TileMetrica.tsx:13-35` (`TendenciaTag`).
...
Taxonomia fechada (CONTEXT.md). Cor: sempre `text-state-lost` (danger). **Não tentar diferenciar visualmente** os motivos por cor — eles são todos perda.
...
| Foto de portaria | `Camera` ou `ImageIcon` | `text-info-500` (info — handoff implícito) | Sim — borda `border-l-info-500`, variante `paused` ("Modelo atendendo") |
...
Aparece em card quando `ia_pausada_motivo === "modelo_em_atendimento"` (pattern em `CardDestaque.tsx:102-110, 180-193`). Confirmação via `AlertDialog`.
...
3. Só criar token novo se for genuinamente novo significado. Adicionar em `globals.css` `:root` + `@theme inline` + `tokens.md` + aqui — tudo no mesmo PR.
